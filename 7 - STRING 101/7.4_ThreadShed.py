...
#Step1
#ordem da string: nome, valor, cor data
...
#split the string around whatever character you replaced the ;,;

for individual in daily_transactions:
  daily_transactions_split.append(individual.split('#'))
#cria um vetor 2d
...
for clean in daily_transactions_split:
  transaction_clean =[]
  for interno in clean:
    transaction_clean.append(interno.replace("\n", "").strip(" "))
...
#Step 11
for index0 in transactions_clean:
  customers.append(index0[0])
  sales.append(index0[1])
  thread_sold.append(index0[2])
...
#Step 16
#we want to determine how many of each color thread we sold today. 
...
#Step 18
for color1 in thread_sold:
  for color2 in color1.split("&"):
...
def color_count(color):
  color_total = 0
  for index in thread_sold_split:
    if color == index:
      color_total += 1
  return color_total
...
#Step 22
for color in colors: 
  print ("Vendeu {0} itens da cor {1}".format (color_count(color), color))
